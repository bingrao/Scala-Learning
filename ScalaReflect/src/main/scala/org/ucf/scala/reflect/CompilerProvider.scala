...
trait TreeCreationMethods {
  ...
  def treeFrom(path:String, isPath:Boolean):global.Tree = if(isPath == true) {
...
object CompilerProvider extends TreeCreationMethods {

  override val global = CompilerInstance.compiler

  /*
  def resetPresentationCompiler() = global.ask { () =>
