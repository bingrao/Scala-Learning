...
trait Trees extends api.Trees {
  ...
  /**
    ...
    *  - Position
    *  - Type (Not for all) default: NoType
    *  - Symbol (Not for all) default: NoSymbol
    *  There are four main categories of tree
    *   - subclasses of TermTree
    *   - subclasses of TypTree
    *   - subclasses of SymTree
    *   - subclasses of NameTree
    */
  abstract class Tree extends TreeContextApiImpl with Attachable with Product {
    ...
    /**
      * Position The pos attribute of the trees is very important for us during refactoring.
      * Unless the compiler runs in IDE mode, it generates OffsetPositions, which have only
      * a single offset – the point – that indicates where a tree comes from; this is sufficient
      * for non-interactive usage. When running in interactive – IDE – mode, the compiler
      * generates RangePositions to represent the source range a tree originally comes from
      * and OffsetPositions for those trees that were compiler-generated. Ranges have a start-,
      * a point- and an end offset, where the end offset points to the position after the last
      * character.
      */
    ...
    /**
      * Symbol Symbols provide another view on the program. Symbols are introduced by
      * DefTrees and referenced by RefTrees. Symbols provide a lot more information about
      * the program than the Trees – there are several dozen isXy methods defined on symbol
      * to query information. In contrast to Trees, Symbols are much more connected to each
      * other: this allows us to resolve class hierarchies or find the enclosing method or class
      * for a symbol. Martin Odersky recorded three videos [Ode09a] that walk through the
      * compiler, explaining many Symbol-related concepts.
      * @return
      */
  ...
  /**
    * The most of operations on a Scala AST, such as filter, find and foreach.
    */
  ...
  /**
    * Subclasses of TermTree which represent terms, e.g., method invocations are represented by Apply nodes,
    * object instantiation is achieved using New nodes, etc.
    */
  trait TermTree extends Tree with TermTreeApi

  /**
    * Subclasses of TypTree which represent types that are explicitly specified in program source code,
    * e.g., List[Int] is parsed as AppliedTypeTree. Note: TypTree is not misspelled, nor is it
    * conceptually the same as TypeTree– TypeTree is something different. That is, in situations
    * where Types are constructed by the compiler (e.g., during type inference), they can be wrapped
    * in TypeTree trees and integrated into the AST of the program.
    * */
  trait TypTree extends Tree with TypTreeApi

  /**
    * Subclasses of SymTree which introduce or reference definitions. Examples of the introduction
    * of new definitions include ClassDefs which represent class and trait definitions, or ValDef
    * which represent field and parameter definitions. Examples of the reference of existing
    * definitions include Idents which refer to an existing definition in the current scope
    * such as a local variable or a method.
    */
  ...
  /**
    * The DefTree class is extended by all trees that define or introduce a new entity into the
    * program. Each DefTree also has a name and introduces a symbol.
    */
  ...
  /**
    * RefTrees represent references to DefTrees. They also have a name and their symbol is
    * the same as their corresponding DefTree’s – i.e. it can be compared using ==.
    */
  ...
  /**
    * Describes a package clause with a package identifier and a list of statements. The
    * package identifier is either an instance of Ident for a package like package a or an
    * instance of Select for a package name like package a.b. The compilation unit root is
    * always a package, even if there is no explicit package declaration present. In this case,
    * the identifier is simply <empty>.
    * @param pid the pacakge name
  ...
  /**
    * The definition for all kinds of classes and traits (objects are defined in ModuleDef).
  ...
  /**
    *  The definition of a singleton object, similar to the ClassDef except that a module does
    *  not take type parameters, because there is no parameters for object.
    * @param mods
    * @param name
    * @param impl
    */
  ...
  /**
    ...
    *   val a:Int                ---->    <stable> <accessor> def a: Int
    *   }                        ----> }
    *
    *   In general, values are always private to the class. For external access, stable accessors
    *   are generated, as the following listing illustrates
    *
    *   class A {                            class A extends Object with ScalaObject {
    *    val a = 42                              private[this] val a: Int = 42;
  ...
  /**
    * The DefDef trees represent method or macro definitions
    ...
    * down the inheritance hierarchy is more expensive. To find all subclasses of a class
    * C one has to collect all other classes in the universe and test each’s ancestors for
    * the presence of C. Once the class hierarchy is assembled, the definition symbol's
    * overriddenSymbol method can be used on each class in the hierarchy to gather all
    * overrides.
    */
  ...
  /**
    * TypeDef trees are definitions of types
    * @param mods
    * @param name
    * @param tparams
    * @param rhs
  ...
  /**
    ...
    * @param name
    * @param params
    * @param rhs  return tree if there are multiple statements in the body then create an additional block that wraps them.
    *
  ...
  /**
    * An import statement imports one or many names – the selectors – from a package or object expr
    * Import trees can also be comma separated, in this case, only the first import includes
  ...
  /**
    *  A Block encloses a list of statements in { . . . } and returns the value of its expr child.
    *  Block trees are only generated when needed – for example, the right hand side of a DefDef with a single
    *  expression is not a Block but the expression itself, even when the expression is enclosed in { . . . }.
    * @param stats a list of statements.
    * @param expr return value, usually the last line of a block. but not always true.
    */
  ...
  /**
    * The body of a Match tree contains a number of CaseDefs trees
    *
    * @param pat  Patterns can be of different form, the catch-all _ is simply an Ident tree, whereas
    *             extractors are represented through the UnApply trees. Patterns that use an @ binding
    *             or are restricted by type with : are Bind trees. The body can again be an arbitrary tree
    * @param guard The guard can be an empty tree if it is not present. Note that even though the
    *              if keyword is used, the tree is not an If tree.
    * @param body The body can again be an arbitrary tree.
    */
  ...
  /**
    * Patterns can choose to match the whole remainder of an extracted sequence using the _* pattern,
  ...
  /**
    * The Bind tree binds a name to an expression and is used in the patterns of CaseDef
    * @param name
    * @param body
    *
    *  list match {                                      list match {
    *     case i => . . .                                    case (i @ _) => . . .
    *     case i: Int => . . .                               case (i @ (_: Int)) => . . .
    *     case a @ i: Int => . . .                           case (a @ (i @ (_: Int))) => . . .
    *   }                                                }
    */
  ...
  /**
    * When an extractor object is used in the pattern of a case definition, an UnApply tree is used.
    * The arguments of UnApply can then be more patterns.
    * @param fun
    * @param args
    *
    *  case Ex(i) => i           case Ex.unapply(<unapply􀀀selector>)
    *                                 <unapply> ((i @ _)) => i
    *  case a @ Ex(i) => i       case (a @ Ex.unapply(<unapply􀀀selector>)
    *                                 <unapply> ((i @ _))) => i
    *  case a @ Ex(i: Int) => i  case (a @ Ex.unapply(<unapply􀀀selector>)
    *                                 <unapply> ((i @ (_: Int)))) => i
    */
  ...
  /**
    * The Function tree contains a single list of parameters and a body for the implementation
    * @param vparams
    * @param body
    *
    * The following listing shows various usages of the Function tree and how their trees look like
    * list foreach println                      list foreach ({ ((x: Any) => println(x)) })
    * list foreach (println _)                  list foreach ({  ((x: Any) => println(x)) })
    * list foreach (i => println(i))            list foreach (((i: Int) => println(i)))
    * list foreach ((i: Int) => println(i))     list foreach (((i: Int) => println(i)))
    * list foreach {  case i => println(i) }    list foreach (((x0$1: Int) => x0$1 match {  case (i @ _) => println(i) }))
    *
    * In the first two examples, the functions are encapsulated in an additional Block
    * hence the curly braces. When the function parameter does not have a name, the
    * compiler generates one and marks it with the SYNTHETIC flag. In the last example,
    * we see that the pattern matching on the parameter is made explicit in the AST
    */
  ...
  /**
    * Assign trees are not used for all = calls, only for non-initial assignments to variables
    * Calls to setter methods are not represented by Assign trees but are regular method calls.
    * @param lhs
    * @param rhs
    */
  ...
  /**
    *
    * @param cond condition part
    * @param thenp then part
    * @param elsep else part,
    * If the else part is omitted, the literal () of type Unit is generated and the type of the
    * conditional is set to an upper bound of Unit and the type of the then expression, usually Any
    * else if terms are implemented using nested if conditionals, for example:
    * if (a) b else if (c) else e ==>  if (a) b else { if (c) else e }
    *
  ...
  /**
    *  A match tree is used to represent a pattern match.
    * @param selector with the selector being the tree that is matched against
    * @param cases
    *
    *  When a pattern matching expression is used as the body of a function, the selector is a synthetic value
    *  list foreach {                         list foreach (((x0$1: Int) => x0$1 match {
    *        case i => println(i)                         case (i @ _) => println(i)
    *  }                                      }
    */
  ...
  /**
    * The Return tree contains an expression that constitutes the return value For return
    * statements without an expression, the compiler generates a () literal.
    * @param expr
    */
  ...
  /**
    * The Typed tree is used whenever an expression is annotated with a type. For example, in the following listing,
    * the second and third occurrences of Int are Typed trees:
    *  val a: Int = 42: Int
    *  println(a: Int)
    *
  ...
  /**
    * TypeApply trees are used whenever a type is applied to a generic method. For example,
    * in the following listing, both expressions on the left are represented by the same AST on the right.
    *  List(1,2,3)                      Apply(
    *                                       TypeApply(
    *  List[Int](1,2,3)                         Select(. . . , "apply"),
    *                                           List(Int)),
    *                                       List(1,2,3))
    *
    * @param fun
  ...
  /**
    *  Function application is represented with Apply trees
    * @param fun is often a Select tree that specifics the function name
    * @param args are the actual parameters
    */
  ...
  /**
    * The Select tree occurs on places that select a name from a qualifier, e.g. in method calls.
    * Note that the typer fully qualifies references as illustrated in the following listing.
    *  class A {                                        class A {
    *        val a = . . .                                   val a = . . .
    *        val b = a                                       val b = A.this.a
    *  }                                                }
  ...
  /**
    * Holds a Name, which can be generated (check with symbol.isSynthetic) by the compiler.
    * Note that the name is in its alphabetic form; the real name can be found via the tree’s symbol.
    * @param name
    */
  ...
  /**
    * An intersection type is represented by a CompoundTypeTree. Note that the tree contains a Template,
    * this is because the compound type can have an optional refinement
    * trait A
    * trait B
    *  .... A with B {
    *     ...
  ...
  /**
    * When a type is applied to a polymorphic function, a TypeApply tree is used. When a type is applied
    * to an other type, an AppliedTypeTree is used
    * @param tpt
    * @param args
    */
  ...
  /**
    * Whenever a type is constrained to lower or upper bounds, TypeBoundsTree represents
    * these bounds. If one of the bounds is omitted, the compiler inserts Nothing respectively
    * Any for the missing lower or upper bound. This is illustrated in the following example
    *
    * type B >: Nothing <: AnyRef                     type B >: Nothing <: AnyRef
    * type C >: String                                type C >: String <: Any
    * type D <: AnyRef                                type D >: Nothing <: AnyRef
    * @param lo
    * @param hi
    */
  ...
  /**
    * Existential types are represented with a ExistentialTypeTree
    * @param tpt
    * @param whereClauses
    */
  ...
  /**
    * A synthetic term holding an arbitrary type. Not to be confused with with TypTree,
    * the trait for trees that are only used for type trees. TypeTrees are inserted in
    * several places, but most notably in RefCheck, where the arbitrary type trees are all replaced by TypeTrees.
    *
    * The original type tree is still accessible via the TypeTree’s original member. Note that
    * the standard tree Traverser and Transformer visitors do not traverse into the original subtree.
    */
  ...
  /**
    * A Tree object that can stand in for most other trees, it has no position, no type and no
    * symbol. For ValDefs, the equivalent is the emptyValDef object.
    */
